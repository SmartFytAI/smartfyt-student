...
export function useAnalytics() {
  ...
  return {
    ...
    // Widget tracking
    trackWidgetInteraction: (widget: string, action: string, properties?: Record<string, any>) =>
    ...
    // Journal tracking
    trackJournalInteraction: (action: string, properties?: Record<string, any>) =>
      trackJournalInteraction(action, withUserContext(properties)),

    // Team tracking
    trackTeamInteraction: (action: string, teamId?: string, properties?: Record<string, any>) =>
      trackTeamInteraction(action, teamId, withUserContext(properties)),

    // Health tracking
    trackHealthInteraction: (action: string, dataType?: string, properties?: Record<string, any>) =>
      trackHealthInteraction(action, dataType, withUserContext(properties)),

    // Coach feedback tracking
    trackCoachFeedbackInteraction: (action: string, properties?: Record<string, any>) =>
