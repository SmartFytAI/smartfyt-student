...
const getCardStyles = (
  ...
  const baseStyles =
    'backdrop-blur-sm border border-gray-200/60 dark:border-gray-700/60 bg-white/90 dark:bg-gray-900/90 transition-all duration-300';
  ...
  const variantStyles = {
    ...
    metric:
      'shadow-md hover:shadow-lg rounded-xl bg-gradient-to-br from-white/95 to-gray-50/95 dark:from-gray-900/95 dark:to-gray-800/95',
    tech: 'shadow-sm hover:shadow-md rounded-lg border-dashed',
  };

  const hoverStyles = isHoverable
    ? 'hover:scale-[1.02] hover:border-primary-200 dark:hover:border-primary-700'
    : '';
...
export function AthleticCard({
  ...
  const cardContent = (
    <Card className={`${cardStyles} ${className}`} onClick={onClick} {...props}>
      <AthleticCardHeader title={title} subtitle={subtitle} icon={icon} />

      <CardContent
        className={`${sizeStyles[size]} ${title || subtitle || icon ? 'pt-0' : ''}`}
      >
...
export function MetricCard({
  ...
  metric: NonNullable<AthleticCardProps['metric']>;
}) {
  return (
    <AthleticCard
      variant='metric'
