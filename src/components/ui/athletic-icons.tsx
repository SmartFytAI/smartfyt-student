...
type IconVariant = 'health' | 'journal' | 'quest' | 'performance' | 'goal' | 'team' | 'tech' | 'auth'
...
export function AthleticIcon({
  ...
  return (
    <div className={`${iconStyles} ${className}`}>
      {children}
    </div>
  )
}
...
export const AthleticIcons = {
  Health: ({ size = 'md' }: { size?: IconSize }) => (
    <AthleticIcon size={size} variant="health">📊</AthleticIcon>
  ),
  Journal: ({ size = 'md' }: { size?: IconSize }) => (
    <AthleticIcon size={size} variant="journal">📝</AthleticIcon>
  ),
  Quest: ({ size = 'md' }: { size?: IconSize }) => (
    <AthleticIcon size={size} variant="quest">🎮</AthleticIcon>
  ),
  Performance: ({ size = 'md' }: { size?: IconSize }) => (
    <AthleticIcon size={size} variant="performance">📈</AthleticIcon>
  ),
  Goal: ({ size = 'md' }: { size?: IconSize }) => (
    <AthleticIcon size={size} variant="goal">🎯</AthleticIcon>
  ),
  Team: ({ size = 'md' }: { size?: IconSize }) => (
    <AthleticIcon size={size} variant="team">👥</AthleticIcon>
  ),
  Tech: ({ size = 'md' }: { size?: IconSize }) => (
    <AthleticIcon size={size} variant="tech">⚛️</AthleticIcon>
  ),
  Auth: ({ size = 'md' }: { size?: IconSize }) => (
    <AthleticIcon size={size} variant="auth">🔐</AthleticIcon>
  ),
  Connection: ({ size = 'md' }: { size?: IconSize }) => (
    <AthleticIcon size={size} variant="tech">🔌</AthleticIcon>
  ),
